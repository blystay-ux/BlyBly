...
export default async function handler(req, res) {
  ...
  // Sign: path + raw JSON body (no escaping — jsStringEscape is identity on valid JSON)
  const payloadToSign = IK_PATH + jsonBody
  const signature     = crypto
    .createHmac('sha256', IK_APP_SECRET.trim())
    .update(payloadToSign, 'utf8')
    .digest('hex')
 
  console.log('[iKhokha DEBUG] ENTITY_ID:', IK_ENTITY_ID ? IK_ENTITY_ID.slice(0,6) + '...' : '(empty — falling back to APP_ID)')
  console.log('[iKhokha DEBUG] APP_ID:', IK_APP_ID.slice(0,4) + '...' + IK_APP_ID.slice(-4))
  console.log('[iKhokha DEBUG] SECRET prefix:', IK_APP_SECRET.trim().slice(0,4))
  console.log('[iKhokha DEBUG] IK-SIGN:', signature.slice(0,8) + '...' + signature.slice(-8))
  console.log('[iKhokha DEBUG] FULL payloadToSign:', payloadToSign)
