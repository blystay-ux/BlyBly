...
const ALL_AMENITIES = ['Pool','WiFi','Spa','Restaurant','Bar','Parking',
  'Gym','Game drives','All-inclusive','Fireplace','Butler service',
  'Horse riding','Wine cellar','Concierge','Air conditioning','Pet friendly']
const STEPS = ['Your details','Property info','Pricing & rates','Review & submit']
...
const s = {
  ...
  label: {
    ...
    color: 'var(--text-muted)', marginBottom: 6, marginTop: 16,
  },
  input: {
  ...
  chip: (on) => ({
    ...
    border: '1.5px solid', borderColor: on ? 'var(--text)' : 'var(--border)',
    background: on ? 'var(--text)' : 'none', color: on ? '#fff' : 'var(--text)',
  ...
  rateHeader: {
    display: 'grid',
    gridTemplateColumns: '1.8fr 130px 130px 100px 36px',
  ...
  reviewVal: { color: 'var(--text)', fontWeight: 600, textAlign: 'right', maxWidth: '60%' },
  rateReviewRow: {
...
export default function ListHotel() {
  ...
  const addRate = () => {
    set('seasonal_rates', [...form.seasonal_rates, {
      label: '', start: '', end: '', price: ''
    }])
  }

  const addPresetSeason = (season) => {
    set('seasonal_rates', [...form.seasonal_rates, {
      label: season.label, start: season.start, end: season.end, price: ''
    }])
  }
  ...
  const removeRate = (i) => {
    set('seasonal_rates', form.seasonal_rates.filter((_, idx) => idx !== i))
  }
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from('hotels').insert({
      ...
      images: imageArr, amenities: form.amenities,
  ...
  if (!user) return (
    <div style={{ ...s.page, textAlign: 'center', paddingTop: 80 }}>
      ...
      <div style={s.sectionSub}>You need an account to list your hotel on Bly.</div>
    ...
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        {step === 1 && <>
          ...
          <div style={s.grid2}>
            ...
            <div>
              ...
              <select style={s.select} value={form.province}
          ...
          <div style={s.amenityGrid}>
            {ALL_AMENITIES.map(a => (
              <button key={a} style={s.chip(form.amenities.includes(a))}
        ...
        {step === 2 && <>
          ...
          <div style={s.sectionSub}>Set your standard rate then add seasonal pricing for the year ahead.</div>
          ...
          <div style={{ position: 'relative', marginBottom: 8 }}>
            <span style={{
              ...
              fontSize: 15, color: 'var(--text-muted)', fontWeight: 600,
            }}>R</span>
            <input style={{ ...s.input, paddingLeft: 32 }}
              type="number" value={form.price_night}
              onChange={e => set('price_night', e.target.value)}
              placeholder="2500" />
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 24 }}>
            This is your default rate — applies to any dates not covered by a seasonal rate below.
          </div>

          {/* Seasonal rates */}
          <label style={s.label}>Seasonal rates (optional)</label>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
            Add different rates for peak periods, holidays, or specials. Dates for 2025–2026:
          </div>

          {/* Quick-add season buttons */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
            {SA_SEASONS.map((s, i) => (
              <button key={i} style={s.quickBtn}
                onClick={() => addPresetSeason(s)}>
          ...
          <button style={s.addRateBtn} onClick={addRate}>
            + Add custom rate period
          </button>
        ...
        {step === 3 && <>
          ...
          {form.seasonal_rates.length > 0 && <>
            <div style={{ ...s.label, marginTop: 20 }}>Seasonal rates</div>
            {form.seasonal_rates.map((r, i) => (
              <div key={i} style={s.rateReviewRow}>
                ...
                <span style={{ fontWeight: 700, color: 'var(--accent)' }}>
                  R{Number(r.price || 0).toLocaleString()}
                </span>
              </div>
            ))}
          </>}
        ...
        <div style={s.btnRow}>
          ...
          {step < 3
            ...
            : <button style={s.btnSubmit} onClick={handleSubmit} disabled={submitting}>
                {submitting ? 'Submitting…' : 'Submit my hotel →'}
              </button>
          }
        </div>
      </div>
